	...
	*/
	SS_RESULT InternalStorage_TruncateFile( const char* group_name, const char* name, uint32_t length, double TIMEOUT )
	{
		...
		if( xSemaphoreTake( InternalStorage_Semphr , TIMEOUT_Ticks ) )
		{
			...
			FRESULT res = f_open(&UserFile, UserFile_filename, FA_OPEN_ALWAYS | FA_WRITE);
			//打开文件
			if(res == FR_OK) 
			{	//写入文件
				uint32_t byteswritten;
				f_lseek( &UserFile, length );
				res = f_truncate(&UserFile);
		...
		uint32_t read_offset, int32_t read_length, double TIMEOUT )
	{
		...
		if( xSemaphoreTake( InternalStorage_Semphr , TIMEOUT_Ticks ) )
		{
			...
			{	//读取文件
				...
				if( res != FR_DENIED )
				{
					f_lseek( &UserFile, read_offset );
					res = f_read(&UserFile, content, read_length, &bytesread);
